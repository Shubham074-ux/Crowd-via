...
{session && <>
<button onBlur={()=>{setTimeout(() => {setShowdropdown(false)
...
<div id="dropdown" className={`z-10 ${showdropdown ?"":"hidden"} absolute left-[180px] bg-gray-600 divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700`}>
    <ul className="py-2 text-sm text-gray-200 dark:text-gray-200" aria-labelledby="dropdownDefaultButton">
      <li>
        <Link  href="/dashboard" className="block px-4 py-2 hover:bg-gray-500 bg-gray-600 hover:text-white">Dashboard</Link>
      </li>
      <li>
        <Link href={`/${session.user.name}`} className="block px-4 py-2 hover:bg-gray-500 bg-gray-600 hover:text-white">Your Page</Link>
      </li>
      
      <li>
        <Link  onClick={()=>signOut()} href="#" className="block px-4 py-2  hover:bg-gray-500 bg-gray-600 hover:text-white">Sign out</Link>
      </li>
    </ul>
</div>
